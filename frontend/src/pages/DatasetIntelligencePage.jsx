  ...
  const fetchDatasetData = async () => {
    ...
    try {
      const [invRes, linRes, metRes] = await Promise.all([
        ...
        axios.get('/api/ml/lineage'),
        axios.get('/api/ml/evaluation-metrics')
      ]);
    ...
    <div className="space-y-8 max-w-7xl mx-auto pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
        <div className="space-y-1">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 text-blue-700 dark:text-cyan-400 border border-blue-500/20 text-xs font-mono font-bold">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 dark:border-slate-800/80 pb-4">
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              <GitBranch className="w-4 h-4 text-blue-600 dark:text-cyan-400" />
        ...
        <div className="lg:col-span-7 p-6 rounded-3xl bg-white dark:bg-[#0B1222] border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800/80 pb-3">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Evaluation truth from LANL Red Team compromise logs (<code className="font-mono text-blue-600 dark:text-cyan-400">redteam.txt.gz</code>).
              </p>
            </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 font-mono text-xs space-y-2">
            <div className="flex items-center justify-between text-slate-700 dark:text-slate-300">
              ...
              <span className="font-bold text-blue-600 dark:text-cyan-400">{metrics?.samples_evaluated?.toLocaleString() || '73,287'}</span>
            </div>
            <div className="flex items-center justify-between text-slate-700 dark:text-slate-300">
        ...
        <div className="lg:col-span-5 p-6 rounded-3xl bg-white dark:bg-[#0B1222] border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col justify-between space-y-4">
          <div className="space-y-3">
            ...
            <p className="text-xs text-slate-600 dark:text-slate-300 leading-relaxed">
              "The anomaly detector operates entirely without knowledge of the attack label. Ground-truth compromise records from <code className="font-mono text-blue-600 dark:text-cyan-400">redteam.txt.gz</code> are held strictly for post-hoc validation."
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="p-6 rounded-3xl bg-white dark:bg-[#0B1222] border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-mono font-bold text-blue-600 dark:text-cyan-400 uppercase">
      ...
      {/* 4. CLEAN 6-D FEATURE SCHEMA DICTIONARY */}
      <div className="p-6 rounded-3xl bg-white dark:bg-[#0B1222] border border-slate-200 dark:border-slate-800 shadow-sm space-y-4">
        <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-800/80 pb-3">
          <FileText className="w-4 h-4 text-blue-600 dark:text-cyan-400" />
          <h2 className="text-base font-bold text-slate-900 dark:text-white">
            6-Dimensional Clean Behavioral Feature Schema (No Red Team Leakage)
          </h2>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            { name: "query_frequency", desc: "Total entity DNS resolution queries within 60-minute sliding window (N)." },
            { name: "unique_destinations", desc: "Number of distinct destination computers resolved by the subject (U)." },
            { name: "destination_entropy", desc: "Shannon entropy of destination distribution: H = -Σ p_i log2(p_i)." },
            { name: "query_rate_per_min", desc: "Communication request velocity computed over active duration window." },
            { name: "destination_fanout_ratio", desc: "Fanout ratio: U / N. Spikes indicate lateral network reconnaissance." },
            { name: "new_destination_ratio", desc: "Fraction of resolved destinations never previously seen for this entity." }
          ].map((f, idx) => (
            <div key={idx} className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-1.5">
